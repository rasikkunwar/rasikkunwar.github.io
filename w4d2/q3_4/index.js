...
app.get('/', (req, res) => {
    res.render('product', { productLists: products, total_cart: req.session.carts ? req.session.carts.length : 0 });
});
...
app.get('/shoppingCart', (req, res) => {
    ...
    res.render('shopping_cart', { cartLists: shoppingCartLists , total_cart: req.session.carts ? req.session.carts.length : 0 });
});
app.post('/addToCart', (req, res) => {
    ...
    res.status(200);
    res.send({total_cart: req.session.carts ? req.session.carts.length : 0 });
})
app.listen(3000, () => {
